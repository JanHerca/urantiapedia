---
title: "Epílogo"
description: "La gloria del mundo chií"
published: true
date: 2024-11-24T12:47:51Z
tags: Islam, book
editor: markdown
dateCreated: 2024-11-24T12:47:51Z
---

<figure class="table chapter-navigator">
  <table>
    <tbody>
      <tr>
        <td>
        <a href="/es/book/Islam/The_Glory_of_the_Shia_World/15">
          <span class="mdi mdi-arrow-left-drop-circle"></span><span class="pl-2">Capítulo XV. La peregrinación es aceptada</span>
        </a>
        </td>
        <td>
        <a href="/es/book/Islam/The_Glory_of_the_Shia_World">
          <span class="mdi mdi-book-open-variant"></span><span class="pl-2">Página de portada</span>
        </a>
        </td>
        <td>
        </td>
      </tr>
    </tbody>
  </table>
</figure>

<span id="p278"><sup><small>[ p. 278 ]</small></sup></span>

<figure id="Figure_27800" class="image urantiapedia image-style-align-center">
<img src="/image/book/Islam/The_Glory_of_the_Shia_World/27800.jpg">
<figcaption>EL SELLO DEL <i>IMÁN</i> RIZA</figcaption>
</figure>

<br style="clear:both;"/>

La inscripción en el sello dice lo siguiente:

Gloria sea a Allah
Yo, el Vice-regente de Allah,
Sultán Abul Hasan Ali,
Hijo de Musa-ar-Riza.

Oh Conquistador de los enemigos!
Oh Jefe de los amigos
¡Oh, fuente de maravillas!
Oh Ali el Elegido!

<span id="p279"><sup><small>[ p. 279 ]</small></sup></span>

<figure id="Figure_27900" class="image urantiapedia image-style-align-center">
<img src="/image/book/Islam/The_Glory_of_the_Shia_World/27900.jpg">
<figcaption>DE UN BOLÍGRAFO LACADO CAJA</figcaption>
</figure>

<br style="clear:both;"/>

_Estaba meditando cómo escribir la fecha de esta obra_
_En verso oculto pero completo,_
_Cuando Hatif (el Buen Ángel) sacó su cabeza y cantó_,
_«Este libro de viajes ha sido completado rápidamente»._ [^70]

<span id="p280"><sup><small>[ p. 280 ]</small></sup></span>

Impreso por R. & R. CLARK, LIMITED, Edimburgo.

<figure id="Figure_27901" class="image urantiapedia image-style-align-center">
<img src="/image/book/Islam/The_Glory_of_the_Shia_World/27901.jpg">
<figcaption>Epigrama persa</figcaption>
</figure>

<br style="clear:both;"/>

<figure class="table chapter-navigator">
  <table>
    <tbody>
      <tr>
        <td>
        <a href="/es/book/Islam/The_Glory_of_the_Shia_World/15">
          <span class="mdi mdi-arrow-left-drop-circle"></span><span class="pl-2">Capítulo XV. La peregrinación es aceptada</span>
        </a>
        </td>
        <td>
        <a href="/es/book/Islam/The_Glory_of_the_Shia_World">
          <span class="mdi mdi-book-open-variant"></span><span class="pl-2">Página de portada</span>
        </a>
        </td>
        <td>
        </td>
      </tr>
    </tbody>
  </table>
</figure>

<br>

## Notas al pie

[^72]: 279:1 _Ninguna obra persa puede concluirse sin que se indique su fecha en un verso, cada letra del cual posee un valor numérico. En el presente caso, el número es 1331 de la Hégira; pero por un recurso poético, la H de Hatif se «elimina» o se deduce, dando como resultado 1326 de la Hégira (1908), que es el año en el que se completó el libro. El texto persa dice lo siguiente:_
