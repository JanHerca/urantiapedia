---
title: Urantiapedia - 支持的语言
description:
published: true
date: 2021-11-28T14:02:03.086Z
tags: help
editor: markdown
dateCreated: 2021-11-28T14:02:03.086Z
---

<figure class="table chapter-navigator">
  <table>
    <tbody>
      <tr>
        <td><a href="/zh/help/phases">项目里程碑</a></td>
        <td><a href="/zh/help">索引</a></td>
        <td><a href="/zh/help/status">当前状态</a></td>
      </tr>
    </tbody>
  </table>
</figure>

## 支持的语言

*《尤兰提亚之书》* 提供以下语言版本：

代码 | 语言 | 标题
--- | --- | ---
ar | Arabic | كِتاب يورانشيا
bg | Bulgarian | Книгата Урантия
cs | Czech | Kniha Urantia
da | Danish | Urantia Bogen
de | German | Das Urantia Buch
el | Greek | Το Βιβλίο της Ουράντια
en | English | The Urantia Book
es | Spanish | El libro de Urantia
et | Estonian | Urantia raamat
fa | Farsi | کتاب یورنشیا
fi | Finnish | Urantia-kirja
fr | French | Le Livre d’Urantia
he | Hebrew | הספר של אורנטיה
hu | Hungarian | Az Urantia könyv
id | Indonesian | Buku Urantia
it | Italian | Il Libro di Urantia
ja | Japanese | ウランティア ブック
ko | Korean | 유란시아 서
lt | Lithuanian | Urantijos Knyga
nl | Dutch | Het Urantia Boek
pl | Polish | Księga Urantii
pt | Portuguese | O Livro de Urântia
ro | Romanian | Cartea Urantia
ru | Russian | Книга Урантии
sv | Swedish | Urantiaboken
zh | Chinese | 玉苒厦之书

语言代码必须遵循 ISO 639-1 标准，每种语言两个字母。https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes

随着《尤兰提亚之书》新译本的面世，它们将被添加到《尤兰提亚百科全书》中，并且支持的语言列表也将扩大。

## 引号

这些是在每种语言中使用的正确引号：

代码 | 语言 | 标准小学和中学 | 替代小学和中学
--- | --- | --- | ---
ar | Arabic | «...» | “...”
bg | Bulgarian | „...“ & ’...’ ‘...’ | «...» & ’...’ ‘...’
cs | Czech | „...“ & ‚...‘ | »...« & ›...‹
da | Danish | »...« "..." & ›...‹ ‘...’ | ”...” & ’...’
de | German | „...“ & ‚...‘ | »...« & ›...‹
el | Greek | «...» & “...” |
en | English | “...” & ‘...’ |
es | Spanish | «...» & “...” | “...” & ‘...’
et | Estonian | „...“ |
fa | Farsi | «...» |
fi | Finnish | ”...” & ’...’ | »...» & ’...’
fr | French | «...» & «...» “...” | ‹...›
he | Hebrew | ”...” & ’...’ | „...” & ‚...’
hu | Hungarian | „...” & »...« |
id | Indonesian | “...” & ‘...’ |
it | Italian | «...» & “...” | “...” & ‘...’
ja | Japanese | 「...」& 『...』 |
ko | Korean | “...” & ‘...’ |
lt | Lithuanian | „...“ & ‚...‘ |
nl | Dutch | “...” & ‘…’ | „...” & ,…’
pl | Polish | „...” | «...» & »...«
pt | Portuguese | «...» & “...” | “...” & ‘...’
ro | Romanian | „...” & «...» |
ru | Russian | «...» & „...“ | “...” & ‘...’
sv | Swedish | ”...” & ’...’ | »...» »...« & ’...’
ar | Chinese | «...» | “...”


https://en.wikipedia.org/wiki/Quotation_mark

## 圣经缩写

圣经缩写必须在 *Urantiapedia Tools* 中注册，才能用于转换过程。请参阅 [缩写](https://github.com/JanHerca/urantiapedia/blob/master/app/abb.js)。这是一个包含 JavaScript 对象的代码文件，其中包含每种语言的缩写列表、每本圣经书卷的标题以及每本书在 Urantiapedia 中的路径。

## 在线翻译

我们用于翻译的一些在线实用程序是：
- https://translate.google.es/
- https://www.deepl.com/translator

## 外部链接

- [Urantiapedia 工具 - GitHub 项目](https://github.com/JanHerca/urantiapedia)
- [Urantiapedia-backup - GitHub 项目](https://github.com/JanHerca/urantiapedia-backup)
- [Urantia基金会](https://www.urantia.org/)
- [蓝色田野](https://blue-fields.netlify.app/)
- [Wiki.js](https://js.wiki/)

<br>

<figure class="table chapter-navigator">
  <table>
    <tbody>
      <tr>
        <td><a href="/zh/help/phases">项目里程碑</a></td>
        <td><a href="/zh/help">索引</a></td>
        <td><a href="/zh/help/status">当前状态</a></td>
      </tr>
    </tbody>
  </table>
</figure>