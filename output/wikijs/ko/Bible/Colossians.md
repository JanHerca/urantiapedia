---
title: 골로새서
description: 
published: true
date: 2021-12-08T13:55:38.362Z
tags: 
editor: markdown
dateCreated: 2021-11-28T13:55:38.362Z
---

<div class="urantiapedia-book-front">
<svg xmlns="http://www.w3.org/2000/svg"
	width="102.6mm" height="136.8mm"
	viewBox="0 0 102.6 136.8" version="1.1">
	<g transform="translate(-7,-5)" style="display:inline">
		<rect style="fill:#e06464;"
			width="9.6" height="136.8" x="7" y="5" />
		<rect style="display:inline;fill:#690404;fill-opacity:1;" width="96.9" height="136.8" x="17" y="5" />
		<text style="font-size:5px;line-height:1.25;font-family: 'Chivo', sans-serif;text-align:center;text-anchor:middle;fill:#fff;stroke-width:0.2" x="61" y="22">성경</text>
		<text style="font-size:4px;line-height:1.25;font-family: 'Chivo', sans-serif;text-align:center;text-anchor:middle;fill:#fff;stroke-width:0.2" x="61" y="125">Korean Revised Version</text>
		<text style="font-size:4px;line-height:1.25;font-family: 'Chivo', sans-serif;text-align:center;text-anchor:middle;fill:#fff;stroke-width:0.2" x="61" y="130">© 1961 by Korean Bible Society</text>
		<text style="font-size:9px;line-height:1.25;font-family: 'Merriweather', serif;text-align:center;text-anchor:middle;fill:#fff;stroke-width:0.2" x="61" y="60">골로새서</text>
	</g>
</svg>
</div>

## 설명


골로새인들에게 보내는 서신은 신약성경의 열두 번째 책입니다. 본문에 따르면 이 편지는 사도 바울과 디모데가 썼으며, 라오디게아 근처의 작은 프리기아 도시이자 소아시아의 에베소에서 약 160km 떨어진 골로새에 있는 교회에 보낸 편지입니다. 

일부 학자들은 점점 더 바울의 저자에 의문을 제기하고 그 편지를 초기 추종자에게 돌리는 대신 그 편지를 신빙성 있는 것으로 옹호합니다. 만일 바울이 저자였다면 아마도 그는 편지(골 4:18)를 기록할 때 아마누엔시스(amanuensis) 또는 비서관을 사용했을 것입니다. 아마도 디모데였을 것입니다. 

## Index

- [장 1](/ko/Bible/Colossians/1)
- [장 2](/ko/Bible/Colossians/2)
- [장 3](/ko/Bible/Colossians/3)
- [장 4](/ko/Bible/Colossians/4)



[성경 전체 색인](/ko/index/bible)


## External links

- https://en.wikipedia.org/wiki/Epistle_to_the_Colossians