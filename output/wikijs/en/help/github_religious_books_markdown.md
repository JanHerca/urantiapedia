...
> In the case of many Bible books, the files with the chapters are in HTML format and not Markdown because they have been generated from files in LaTeX format that are located within the project in `/input/tex`. This automation is due to the fact that many of these books have been obtained by export from the software [e-Sword](https://www.e-sword.net/), which generates content similar to LaTeX. This task is detailed in the manual section [Getting the Bible](/en/help/github_bible). This task has been completed for the 28 languages ​​currently supported.
